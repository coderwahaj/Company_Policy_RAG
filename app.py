...
#             answer, sources, context_preview, status = run_rag(query, embedder, vector_store, llm, reranker)
...
#             # Check if the query contains any small-talk keyword
#             if any(trigger in query.lower() for trigger in small_talk_triggers):
#                 response_text = llm.generate(query, casual=True)  # normal friendly answer
#             else:
...
#         # Use pipeline objects stored in session
#         embedder, vector_store, llm, reranker, _ = st.session_state.pipeline_objects

#         with st.spinner("Thinking…"):
#             answer, sources, context_preview, status = run_rag(
#                 query, embedder, vector_store, llm, reranker
#             )
