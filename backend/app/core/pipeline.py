...
def pipeline_status(provider: str = "groq", data_root="data/policy"):
    """
    Return whether provider pipeline is currently in in-memory cache.
    Also returns chunk_count when available.
    """
    fp = compute_data_fingerprint(data_root)
    key = (provider, fp)
    ...
    with _PIPELINE_LOCK:
        ...
        # tuple: (embedder, vs, llm, reranker, bm25, chunk_count)
        chunk_count = 0
