...
class TestClassifyQuery:
    ...
    def test_classify_casual_query_with_punctuation(self):
        ...
        for query in punctuated_queries:
            ...
            assert result in ["casual", "unknown"], f"Failed for query: '{query}' - got {result}"
...
class TestRunRag:
    ...
    def test_run_rag_identity_query(self, mock_embedder, mock_vector_store, mock_llm, mock_reranker, mock_bm25):
        """Test run_rag handles identity queries"""
        from backend.app.core.rag_service import run_rag
        
        with patch("backend.app.core.rag_service.classify_query", return_value="identity"):
    ...
    def test_run_rag_casual_query(self, mock_embedder, mock_vector_store, mock_llm, mock_reranker, mock_bm25):
        """Test run_rag handles casual queries"""
        from backend.app.core.rag_service import run_rag
        
        mock_llm.generate = Mock(return_value="Hello! How can I help?")
        
        with patch("backend.app.core.rag_service.classify_query", return_value="casual"):
            answer, sources, context, status = run_rag(
                "Hello",
                mock_embedder,
                mock_vector_store,
    ...
    def test_run_rag_policy_query_no_results(self, mock_embedder, mock_vector_store, mock_llm, mock_reranker, mock_bm25):
        """Test run_rag returns default message when no results"""
        from backend.app.core.rag_service import run_rag
        
        mock_reranker.rerank = Mock(return_value=[])
        
        with patch("backend.app.core.rag_service.classify_query", return_value="policy"):
            with patch("backend.app.core.rag_service.rewrite_query", return_value="rewritten"):
                answer, sources, context, status = run_rag(
                    "Leave policy?",
                    mock_embedder,
                    mock_vector_store,
    ...
    def test_run_rag_policy_query_success(self, mock_embedder, mock_vector_store, mock_llm, mock_reranker, mock_bm25):
        """Test run_rag successfully processes policy query"""
        from backend.app.core.rag_service import run_rag
        
        mock_llm.generate = Mock(return_value="Based on policy, employees get 20 leave days.")
    ...
    def test_run_rag_with_history(self, mock_embedder, mock_vector_store, mock_llm, mock_reranker, mock_bm25):
...
class TestRunRagStream:
    ...
    def test_run_rag_stream_identity_query(self, mock_embedder, mock_vector_store, mock_llm, mock_reranker, mock_bm25):
        """Test run_rag_stream handles identity queries"""
        from backend.app.core.rag_service import run_rag_stream
        
        with patch("backend.app.core.rag_service.classify_query", return_value="identity"):
            events = list(run_rag_stream(
                "Who are you?",
                mock_embedder,
                mock_vector_store,
                mock_llm,
    ...
    def test_run_rag_stream_casual_query(self, mock_embedder, mock_vector_store, mock_llm, mock_reranker, mock_bm25):
    ...
    def test_run_rag_stream_policy_query(self, mock_embedder, mock_vector_store, mock_llm, mock_reranker, mock_bm25):
    ...
    def test_run_rag_stream_yields_proper_format(self, mock_embedder, mock_vector_store, mock_llm, mock_reranker, mock_bm25):
